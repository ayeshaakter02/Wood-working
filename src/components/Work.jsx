    ...
    <div className="relative w-full mx-auto overflow-hidden" id="work">
      <div ref={sliderRef} className="flex overflow-hidden scroll-smooth">
        {images.map((item, index) => (
          <div
            key={index}
            className="w-1/3 flex-shrink-0 py-2 cursor-pointer"
            onClick={() => navigate(item.link)}
          >
            <div className="w-[600px] h-[714px] bg-linear-to-t from-[#030303] to-[#ffa86a59] m-5 py-12 px-15 rounded-xl border border-[#382013] shadow-lg overflow-hidden hover:scale-105 transition">
              <img src={item.src} alt={item.title} className="mx-auto" />
              <div>
                <h3 className="text-white font-forum font-normal text-[67px] tracking-[-3px] leading-[75px] flex justify-center py-1.5 -mt-5 hover:text-[#ffa86a]">
                  {item.title}
                </h3>
                <p className="text-sm text-[#ffa86a] font-semibold tracking-[1px] leading-[60px] flex justify-center align-top font-inter">
                  {item.description}
                </p>
              </div>
            </div>
